...
If this is an existing project, read `.planning/codebase/index.json` to understand what audit documents are available and what each one covers. Do not read the audit documents themselves yet — load them on demand in Step 4. If this is a new project with no existing codebase, skip this — there is no audit.
